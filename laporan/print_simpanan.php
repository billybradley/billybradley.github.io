...
$pdf->SetFont('Arial','',10);
	...
	while($data=mysqli_fetch_array($query)){
    	...
    	$d=mysqli_fetch_array(mysqli_query($connect, "SELECT SUM(besar_simpanan) as pokok from t_simpan where kode_anggota='$kode_ang' and jenis_simpan='pokok'"));
		$e=mysqli_fetch_array(mysqli_query($connect, "SELECT SUM(besar_simpanan) as wajib from t_simpan where kode_anggota='$kode_ang' and jenis_simpan='wajib'"));
		$f=mysqli_fetch_array(mysqli_query($connect, "SELECT SUM(besar_simpanan) as sukarela from t_simpan where kode_anggota='$kode_ang' and jenis_simpan='sukarela'"));
...
$no++;}
$a=mysqli_fetch_array(mysqli_query($connect, "SELECT SUM(besar_simpanan) as total from t_simpan"));
