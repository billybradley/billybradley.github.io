...
<div class="row mt">
                <div class="col-lg-12">
                    ...
                    <?php
                        $am=mysqli_query($connect, "select*from t_anggota");
...
<?php
	$query = mysqli_query($connect,"SELECT * from t_anggota");
	echo '<tbody id="fbody">';
	$no=1;	
...
?>
    	<tr>
			...
			<?php $d=mysqli_fetch_array(mysqli_query($connect,"SELECT SUM(besar_simpanan) as pokok from t_simpan where kode_anggota='$kode_ang' and jenis_simpan='pokok'")); ?>
			<td>Rp. <?php echo number_format($d['pokok']);?></td>
			<?php $e=mysqli_fetch_array(mysqli_query($connect,"SELECT SUM(besar_simpanan) as wajib from t_simpan where kode_anggota='$kode_ang' and jenis_simpan='wajib'")); ?>
			<td>Rp. <?php echo number_format($e['wajib']);?></td>
			<?php $f=mysqli_fetch_array(mysqli_query($connect,"SELECT SUM(besar_simpanan) as sukarela from t_simpan where kode_anggota='$kode_ang' and jenis_simpan='sukarela'")); ?>
...
<?php
	...
	$q=mysqli_query($connect, "SELECT S.*, A.nama_anggota FROM t_simpan S, t_anggota A
...
<form class="form-inline" role="form">
  <table class="table table-bordered table-striped table-condensed">
    ...
    <tbody>
<?php
$query = mysqli_query($connect, "SELECT * from t_simpan where kode_anggota='$kode'");
	echo '';	
	$no=1;
	while($data=mysqli_fetch_array($query)){
?>
...
<tr  class="info"><td colspan="3" align="center">Total</td>
  <td>Rp. <?php $bu=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_simpanan) as besar_simpan from t_simpan where kode_anggota='$kode'")); echo number_format($bu['besar_simpan']);
  echo '</td>';?>
</tr>
</tbody>   
</table>
