...
$query=mysqli_query($connect,"SELECT * FROM t_tabungan");
	$no=1;
	while($data=mysqli_fetch_array($query))
	{
	    ...
	    $d=$data['kode_anggota'];$f=mysqli_fetch_array(mysqli_query($connect,"SELECT nama_anggota from t_anggota where kode_anggota='$d'"));
	    $pdf->Cell(5, 0.8, $f['nama_anggota'],1, 0,'C');
		$pdf->Cell(5, 0.8, number_format($data['besar_tabungan']),1, 1,'C');
		$no++;
	}
$hasil=mysqli_fetch_array(mysqli_query($connect,"SELECT sum(besar_tabungan) as besar from t_tabungan"));
