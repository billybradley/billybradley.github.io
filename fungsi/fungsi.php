...
{
	global $connect;
	$struct = mysqli_query($connect, "SELECT * FROM $tabel");
	$field  = mysqli_field_name($struct, 0);
	$len    = mysqli_field_len($struct, 0);

	$qry = mysqli_query($connect, "SELECT max(" . $field . ")FROM " . $tabel);
...
function nomer($initial, $field, $table)
{
	...
	$sql = mysqli_query($connect, "SELECT $field FROM $table ORDER BY $field DESC LIMIT 1 ");
