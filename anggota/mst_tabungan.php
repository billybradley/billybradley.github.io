...
if(empty($aksi))
{
    $ghu=mysqli_query($connect, "SELECT * FROM t_tabungan");
$no=1;
while($dataku=mysqli_fetch_array($ghu))
{
  ...
  if($tempo==$tang)
  {
    ...
    $tol=mysqli_query($connect, "UPDATE t_tabungan set tgl_mulai='$tang',besar_tabungan='$total' where kode_tabungan='$kode_tab'");
  }
  else
  {
 ...
 <div class="col-lg-12">
  <div class="form-panel">
   <h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Data Tabungan
     <?php
      $am=mysqli_query($connect, "select*from t_tabungan");
...
<?php
	$query=mysqli_query($connect, "SELECT * FROM t_tabungan order by kode_tabungan desc");
	$no=1;
...
?>
    	...
    	<tr>
            ...
            $d=$data['kode_anggota'];$f=mysqli_fetch_array(mysqli_query($connect, "SELECT nama_anggota from t_anggota where Kode_anggota='$d'")); $rty=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_simpanan) as total_asli from t_simpan where Kode_anggota='$d'")); $inves=$data['besar_tabungan']-$rty['total_asli'];?>
...
<tr  class="info"><td colspan="5" align="center">Total</td>
  <td colspan="2">Rp. <?php $bu=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_tabungan) as besar_tab from t_tabungan")); echo number_format($bu['besar_tab']);
  echo '</td>';?>
...
<div class="row mt">
    ...
    <form action="anggota/proses_tabungan.php" method="get">
      <div class="form-group">
        ...
        <div class="form-group">
            <label>Saldo</label>
            <?php $rtyu=mysqli_fetch_array(mysqli_query($connect, "SELECT *FROM t_tabungan where kode_tabungan='$lo' and kode_anggota='$luy'")); ?>
...
{ $lo=$_GET['kode_tabungan'];$luy=$_GET['kode_anggota'];$nama=mysqli_fetch_array(mysqli_query($connect, "SELECT nama_anggota from t_anggota where Kode_anggota='$luy'")); ?>
 <div class="row mt">
 <div class="col-lg-12">
  <div class="form-panel">
...
<?php
  $query=mysqli_query($connect, "SELECT * FROM t_pengambilan where kode_anggota='$luy' and kode_tabungan='$lo' order by kode_ambil desc");
  $no=1;
  while($data=mysqli_fetch_array($query)){
?>
...
<tr  class="info"><td colspan="5"a align="center">Total</td>
  <td colspan="2">Rp. <?php $bu=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_ambil) as besar_ambil from t_pengambilan where kode_anggota='$luy'")); echo number_format($bu['besar_ambil']);
  echo '</td>';?>
</tr>
