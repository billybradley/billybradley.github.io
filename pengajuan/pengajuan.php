...
include "config/koneksi.php";
    $sqlku=mysqli_query($connect, "SELECT * from t_pengajuan order by kode_pengajuan desc");
  $no=1;
...
?>
      ...
      $isi=$data['kode_anggota'];
       $ang=mysqli_fetch_array(mysqli_query($connect, "SELECT * from t_anggota where kode_anggota='$isi'")); ?>
    ...
    <?php $oo=mysqli_fetch_array(mysqli_query($connect, "SELECT*from t_jenis_pinjam where kode_jenis_pinjam='$rr'"));?>
      <?php
...
<div class="row mt">
 <div class="col-lg-12">
  <div class="form-panel">
   <h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Data Pengajuan 
   <?php $dfl=mysqli_fetch_array(mysqli_query($connect, "SELECT *from t_anggota where kode_anggota='$kodeaa'")); echo $dfl['nama_anggota']; ?><span style="float:right;">
    <input type="text" id="cari" style="width:230px;height:30px;font-size:15px;" placeholder=" cari disini...">
                    
   <?php  $verida=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM t_pengajuan where kode_anggota='$kodeaa' order by kode_pengajuan desc")); $num=mysqli_num_rows($verida);
  ...
  <table class="table table-bordered table-striped table-condensed">
    ...
    <tbody id="fbody">
<?php
    $rino=mysqli_query($connect, "SELECT * FROM t_pengajuan where kode_anggota='$kodeaa' order by kode_pengajuan desc");
  $no=1;
...
<div class="form-group">
 ...
 $kdp=mysqli_fetch_array(mysqli_query($connect, "SELECT kode_pengajuan from t_pengajuan order by kode_pengajuan desc"));
