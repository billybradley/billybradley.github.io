...
$maks=$_GET['maks_pinjam'];
        $simpan=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_simpanan) as simpan from t_simpan"));
       	//echo $tabung['tabung'];
       	$angsur=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_angsuran) as angsuran from t_angsur"));
       	//echo $angsur['angsuran'];
       	$denda=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(denda) as denda from t_angsur"));
       	//echo $denda['denda'];
       	$pinjam=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_pinjam) as pinjam from t_pinjam"));
       	$ambil=mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_ambil) as ambil from t_pengambilan"));
...
else if($sisa>$besar_pinjaman)
{
	...
	$sql=mysqli_query($connect, "INSERT into t_pinjam values ('','$kode_anggota','$kode_jenis_pinjam','$besar_pinjaman','$besar_angsuran','$lama_angsuran','','$besar_pinjaman','$u_entry','$tgl_entri','$tempo','belum lunas')");
	if($sql)
	{ ?>
		<script>
