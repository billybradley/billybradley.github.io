...
?>
				...
				<div class="form-panel">
					...
					<form class="form-inline" role="form">
						<table class="table table-bordered table-striped table-condensed">
							...
							<?php
							$query = mysqli_query($connect, "SELECT * FROM t_anggota where status='aktif'
	...
	$nama = mysqli_fetch_array(mysqli_query($connect, "SELECT *from t_jenis_simpan where kode_jenis_simpan='$kode_jenis'"));
	$qubah = mysqli_query($connect, "SELECT * FROM t_anggota WHERE kode_anggota='$kode'");
				...
				<div class="form-panel" style="width:50%;">
					...
					<form action="transaksi/proses_transaksi.php?pros=simpan" method="post" id="form" name="mainform" onSubmit="validasiSimpan()">
						...
						<div class="form-group">
							...
							<input type="text" name="user_entri" class="form-control" size="54" value="<?php session_start();
																										echo $_SESSION['kopname']; ?>" readonly>
						</div>
						<div class="form-group">
					...
					<form action="transaksi/proses_pinjam.php" method="GET" id="form" name="frmAdd">
						...
						<div class="form-group">
							...
							<select name="kode_jenis_pinjam" class="form-control" id="kode_jenis_pinjam" onChange="show3(this.value)" class="required" title="Jenis Pinjaman harus diisi">
								<?php
								$q = mysqli_query($connect, "SELECT * FROM t_jenis_pinjam");
						...
						<div class="form-group">
							<label>User Entri</label>
							<input type="text" class="form-control" name="u_entry" size="54" value="<?php session_start();
																									echo $_SESSION['kopname']; ?>" readonly>
						</div>
						<div class="form-group">
							<label>Tanggal Entri</label>
	...
	$jio = mysqli_fetch_array(mysqli_query($connect, "SELECT * from t_pinjam where kode_pinjam='$kodep' "));
			...
			<div class="col-lg-12">
				<div class="form-panel" style="width:50%;">
					...
					<form action="transaksi/proses_transaksi.php?pros=angsur" method="post" id="form" name="frmAdd">
						...
						<?php
						$kk = mysqli_fetch_array(mysqli_query($connect, "SELECT * from t_pinjam where kode_pinjam='$kodep' and kode_anggota='$kode'"));
						$tempo = $kk['tgl_tempo'];
						$dat = date("Y-m-d");
						if ($dat > $tempo) {
				...
				$kode = $_GET['kode_anggota'];
				$anggota = mysqli_fetch_array(mysqli_query($connect, "SELECT nama_anggota from t_anggota where kode_anggota='$kode'")); ?>
		<div class="row mt">
			<div class="col-lg-12">
				<div class="form-panel">
					<h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Transaksi <?php echo $anggota['nama_anggota']; ?>
						...
						$am = mysqli_query($connect, "select*from t_pinjam where kode_anggota='$kode'");
						$jum = mysqli_num_rows($am);
						echo '<kbd style="background-color:#d9534f;">' . $jum . '</kbd>'; ?>
						<span style="float:right;">
							<?php $df = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM t_pinjam where kode_anggota='$kode' order by kode_pinjam desc"));
							$op = mysqli_num_rows($df);
					...
					<form class="form-inline" role="form">
						<table class="table table-bordered table-striped table-condensed">
							...
							<tbody>
								<?php $sql = mysqli_query($connect, "SELECT * from t_pinjam where kode_anggota='$kode' order by kode_pinjam desc");
								$nomer = 1;
								while ($data = mysqli_fetch_array($sql)) {
									$kd_a = $data['kode_anggota'];
									$kd_j = $data['kode_jenis_pinjam'];
									$jenis = mysqli_fetch_array(mysqli_query($connect, "SELECT nama_pinjaman from t_jenis_pinjam where kode_jenis_pinjam='$kd_j'"));
									echo '<tr>
    		<td>' . $nomer . '</td>
			...
			<td>';
									echo $data['sisa_angsuran'] . ' Bulan Dari ' . $data['lama_angsuran'];
									echo ' Bulan</td>
			...
			<a class="btn btn-primary btn-xs" href=index.php?pilih=3.3&aksi=show&kode_anggota=' . $data['kode_anggota'] . '&kode_pinjam=' . $data['kode_pinjam'] . '>View</a> ';
									$dfo = mysqli_num_rows(mysqli_query($connect, "SELECT *from t_angsur where kode_pinjam='$kd_p' and kode_anggota='$kode'"));
	...
	<?php } elseif ($aksi == 'simpanananggota') {
				$kode = $_GET['kode_anggota'];
				$q = mysqli_query($connect, "SELECT *from t_anggota where kode_anggota='$kode'");
			...
			<div class="col-lg-12">
				<div class="form-panel">
					<h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Laporan Simpanan Anggota "<?php echo $ang['nama_anggota']; ?>"
						<?php
						$am = mysqli_query($connect, "SELECT * FROM t_simpan where kode_anggota='$kode'");
						$jum = mysqli_num_rows($am);
						echo '<kbd style="background-color:#d9534f;">' . $jum . '</kbd>'; ?>
						<span style="float:right;">
							<?php
							$rino = mysqli_query($connect, "SELECT*FROM t_jenis_simpan");
							$no = 1;
							while ($verida = mysqli_fetch_array($rino)) {
								if ($verida['nama_simpanan'] == 'wajib') {
									$baru = mysqli_fetch_array(mysqli_query($connect, "SELECT *FROM t_simpan where kode_anggota='$kode' and jenis_simpan='wajib' order by kode_simpan desc "));
									$numrow = mysqli_num_rows($baru);
									$data = $baru['tgl_mulai'];
									$now = date("Y-m-d");
									if ($data == $now) {
								...
								} else if ($verida['nama_simpanan'] == 'pokok') {
									echo '<a href="index.php?pilih=2.1&aksi=simpan&kode_anggota=' . $kode . '&kode_jenis_simpan=' . $verida['kode_jenis_simpan'] . '" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span> Tambah Simpanan</a>';
								}
								$no++;
							}
							?>
					...
					<form class="form-inline" role="form">
						<table class="table table-bordered table-striped table-condensed">
							...
							<?php
							$query = mysqli_query($connect, "SELECT * from t_simpan where kode_anggota='$kode'order by kode_simpan desc");
							echo '<tbody>';
							$no = 1;
							while ($data = mysqli_fetch_array($query)) {
							?>
							...
							<tr class="info">
								...
								<td>Rp. <?php $bu = mysqli_fetch_array(mysqli_query($connect, "SELECT sum(besar_simpanan) as besar_simpan from t_simpan where kode_anggota='$kode'"));
