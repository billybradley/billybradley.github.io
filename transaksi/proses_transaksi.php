...
$j=mysqli_fetch_array(mysqli_query($connect, "SELECT * from t_jenis_simpan where kode_jenis_simpan='$kode_jenis_simpan'"));
$jenis_simpan=$j['nama_simpanan'];
if($jenis_simpan=='wajib')
{
	$ambil=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM t_simpan where kode_anggota='$kode_anggota' and jenis_simpan='wajib' order by kode_simpan desc "));$num=mysqli_num_rows($ambil);
	if($ambil<=0)
	{
		$mulai=date("Y-m-d");
...
$pros=$_GET['pros'];
	...
	switch($pros){
		case "simpan"	:
							$qtambah=mysqli_query($connect, "INSERT INTO t_simpan VALUES('','$jenis_simpan','$besar_simpanan','$kode_anggota','$user_entri','$banding','$tgl_entri')");
							$sqlbaru=mysqli_fetch_array(mysqli_query($connect, "SELECT besar_tabungan from t_tabungan where kode_anggota='$kode_anggota'"));
							$hasil=$sqlbaru['besar_tabungan']+$besar_simpanan;
							$q=mysqli_query($connect, "UPDATE t_tabungan SET besar_tabungan ='$hasil' 
		...
		case "angsur"	:	$sql=mysqli_fetch_array(mysqli_query($connect, "SELECT sisa_angsuran from 									t_pinjam where kode_pinjam='$kode_pinjam'"));
							$sisa_angsur=$sql['sisa_angsuran']-1;
							$sqla=mysqli_fetch_array(mysqli_query($connect, "SELECT * from t_pinjam where kode_pinjam='$kode_pinjam'"));$sisapinjam=$sqla['sisa_pinjaman'];
							$sqlp=mysqli_fetch_array(mysqli_query($connect, "SELECT besar_angsuran from t_pinjam where kode_pinjam='$kode_pinjam'"));$besarangsur=$sqlp['besar_angsuran'];
							...
							{
								mysqli_query($connect, "UPDATE t_pinjam set sisa_angsuran='$angsuran_ke',sisa_pinjaman='$siso',status='lunas' where kode_pinjam='$kode_pinjam'");
							}
							else
							{
								mysqli_query($connect, "UPDATE t_pinjam set sisa_angsuran='$angsuran_ke',sisa_pinjaman='$siso',tgl_tempo='$lagitempo' where kode_pinjam='$kode_pinjam'");
							}
							
							$aaa=mysqli_fetch_array(mysqli_query($connect, "SELECT lama_angsuran from t_pinjam where 						kode_pinjam='$kode_pinjam'"));
							
							$s=mysqli_query($connect, "INSERT INTO t_angsur VALUES('','$kode_pinjam','$angsuran_ke','$besarangsu','$denda','$siso','$kode_anggota','$u_entry','$tgl_entri')");
							if($s)
							{ ?>
								<script>
